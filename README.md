...
### Информация о покупателе IContactForm
...
### Успешное действие  ISuccessActions
...
### Использование компьютерной мышки
...
- setCatalog - формирует каталог продуктов, преобразуя каждый элемент в экземпляр;
- setPreview - предпросомотр продукта;
- setNewField - обновляет поле формы заказа, проверяет валидность заказа;
...
Конструктор: (container: HTMLElement, protected events: IEvents) обработчик событий корзины;

Методы: 
- catalog - показывает список товаров;
- counter - показывает список товаров в корзине;
- locked - показывает состояние блокировки страницы;


### Класс Card 

Базовая функциональность: отображение карточек, взятых из каталога; Наследуется от Component; использует данные ICardItem;
...
Конструктор: (container: HTMLElement, actions?: ICardAction) принимает контейнер карточки товара, а так же инпут "категории товара";
...
Базовая функциональность: устанавливает адрес доставки товара пользователя и оплату заказа; наследуется от Form;

Конструктор: (container: HTMLFormElement, events: IEvents) принимает контактные данные пользователя и объект событий;
